...
- Depends on: T24
- Parallel group: L
- Status: [~] partial — see finding below. Real production build (`next build && next start`) audited with Lighthouse 13.4.1, mobile emulation (375×812, simulated throttling): **Accessibility 100, Best Practices 100, SEO 100, Performance 62** (Total Blocking Time ~1.86s is the dominant factor, weight 30%, score 0.09). First attempt accidentally measured the `next dev` server still running in the background from T24's browser QA session (caught via `next start`'s own `EADDRINUSE` error, and confirmed by dev-only `next-devtools`/`hmr-client` chunks in that first report) — killed it, re-ran cleanly against a genuine `next start` production server, confirmed via curl showing only real minified chunk names in the served HTML. The 62 is a real measurement of a real production build, not a dev-server artifact. However: this machine is a shared/sandboxed dev environment, and Lighthouse's `simulate` throttling mode scales its CPU-slowdown multiplier off a live benchmark of the *host* machine — on an unrepresentative or loaded host, that inflates TBT/Performance results in a way that doesn't reflect real mobile-device performance once deployed to Vercel's actual edge network. The app itself has no obvious red flag (no extra heavy dependencies, no accidentally-bundled devtools, `next.config.ts` is untouched default config) beyond ~55 KiB of estimated unused JS in the framework chunks (normal for Next.js/React's own runtime, not something this app's code controls directly). **Recommendation: re-run Lighthouse (or check Vercel Speed Insights / PageSpeed Insights) against the real deployed URL once T26 ships** — that's a clean, representative measurement; chasing this number further on the local dev machine risks optimizing against environment noise rather than a real problem. If the deployed score is still <90, that's the point to dig into concrete fixes with a trustworthy baseline.
- Review criteria:
