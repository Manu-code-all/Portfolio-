...
Status: shipped · Progress: 27/27 tasks (T27 added post-ship) — live at https://portfolio-theta-ten-ez943ji5sr.vercel.app
...
- Status: [x] done (resolved 2026-09-01, after T26 deploy)

**Resolution:** first pass (local `next start`) measured Performance 62, dominated by Total Blocking Time ~1.86s — flagged as likely sandboxed-host CPU noise rather than a real app problem (see finding logged earlier in this file and in MEMORY.md). Re-ran Lighthouse against the real deployed URL after T26 shipped: **Performance 73, Accessibity 100, Best Practices 100, SEO 100. Total Blocking Time dropped to 30ms (score 1.0)** — confirming the local TBT problem really was host-CPU noise, not app code. The remaining gap to 90 is now FCP (3.3s) / LCP (4.5s) / Speed Index (6.0s), and the network trace shows tiny files (30KB fonts, 7KB CSS) taking 2+ seconds to transfer even though the root document itself responds in 30ms server-side — a strong signal this is *also* an environment artifact (this sandbox's outbound network path to Vercel's edge), not a real site defect: a real browser tab loaded the same URL and rendered instantly (verified live, screenshot taken). Recommendation for final confirmation: the user (or a future session) should run PageSpeed Insights (pagespeed.web.dev) against the live URL from a normal network — that runs Lighthouse from Google's own infrastructure and would settle this definitively. Marked done rather than left pending because every actionable, in-app-control finding (a11y, best-practices, SEO, TBT/main-thread work) is resolved and verified; the remaining gap is environment-measurement uncertainty, not a known code defect to fix.

**Final confirmation (2026-09-01, same day):** ran PageSpeed Insights against the live URL, from Google's own infrastructure — the fully authoritative, non-sandboxed measurement. Result: **Performance 97, Accessibility 100, Best Practices 100, SEO 100** (Moto G Power emulation, Slow 4G throttling — FCP 0.9s, LCP 2.0s, TBT 20ms, CLS 0, Speed Index 3.9s). This fully confirms both earlier hypotheses: the original local-machine TBT problem and the deployed-measurement FCP/LCP problem were both artifacts of this sandbox's CPU/network environment, not the app. The PRD's ≥90 mobile Performance target is met with real margin. No further performance work needed.
...
**Resolution:** user connected their own Vercel account, imported `Manu-code-all/Portfolio-`, deployed with default Next.js settings (no env vars needed — static site). Live at `https://portfolio-theta-ten-ez943ji5sr.vercel.app`. Branch naming tidied up same day: GitHub's default branch switched to `main` (via `gh repo edit --default-branch main`), and the user switched Vercel's Production Branch setting to `main` in the dashboard. `mastermind/portfolio` is kept around (not deleted) with identical content, in sync via merge. Full verification, all real: homepage 200, `/resume.pdf` 200 (`content-type: application/pdf`), `/robots.txt`/`/sitemap.xml`/OG-Twitter meta tags all resolve to the real domain (updated from T21/T22's placeholder — see commit), all 4 profile links + 3 project repo links resolve (LinkedIn and LeetCode return non-200 to a bare `curl` because both sites block unauthenticated automated requests — verified for real via an actual browser tab: LinkedIn showed its standard logged-out profile gate ("Not you?", confirming a real profile), LeetCode's tab title read "manug9868 - LeetCode Profile").

### T27 — Spectacular redesign (post-ship)
- Feature: user request, 2026-09-01, after seeing the shipped site — "not liking the ui," asked for a Kokonut UI / bklit / motion.dev-inspired redesign
- Files: `app/globals.css`, `components/ui/Reveal.tsx` (new), `components/ui/ParticleButton.tsx` (new), `components/sections/Hero.tsx`, `components/sections/About.tsx`, `components/sections/Skills.tsx`, `components/sections/Achievements.tsx`, `components/sections/Projects.tsx`, `components/projects/ProjectCard.tsx`, `docs/DESIGN.md` §8-9, `package.json` (added `motion`)
- Domain: nextjs
- Depends on: T26 (redesign happens on the already-shipped site)
- Status: [x] done (2026-09-01)

**Resolution:** researched all three referenced sites (kokonutui.com, bklit.com, motion.dev) before building anything. Findings reported back to the user: Kokonut's flagship components (glassmorphism, particles, shimmer, AI-prompt widgets) directly conflicted with `DESIGN.md`'s own explicit bans; Bklit is a chart/data-viz library with no real data in this project's content model to visualize; motion.dev (the animation library) was a clean fit with no conflict. Built a live two-direction comparison as an artifact (same real hero + one project card, "Restrained" vs "Spectacular" toggle) so the user could see before choosing rather than guessing from a description — user picked Spectacular.

Implemented across the real site: `.glass-surface`/`.shimmer-text`/`.ambient-glow` utilities and a canvas-based `ParticleButton`, all built from the existing Warm Graphite/Signal Rust/Bone tokens (never Kokonut's own purple/glass defaults) — see `DESIGN.md` §9 for the full inventory. Added `motion` (motion.dev's React package) for `whileInView` scroll reveals across About/Projects/Skills/Achievements via a new `Reveal` component.

Two real bugs caught and fixed before shipping: (1) the ambient-glow blob, if added naively, overflowed its section horizontally at every breakpoint — fixed by giving `.hero` `position:relative; overflow:hidden` and explicit `z-index` stacking so glow stays behind content and clipped to its section (same overflow-bug class as T24's, now a standing thing to check any time a decorative absolutely-positioned element is added); (2) `.diagram-box`/`.diagram-line` are shared classes used by the hero graphic, the small collapsed-state project preview icons, AND the big expanded architecture diagrams — glass-ifying the hero graphic's box fills without also glass-ifying `.diagram-figure` (the expanded-detail diagram wrapper) would have left translucent boxes floating with no blur backdrop wherever a diagram renders outside a glass container; fixed by glass-ifying `.diagram-figure` too so every context using those shared classes has a consistent backdrop.

Verified: lint/tsc/build all clean, zero horizontal overflow at 375/768/1440px, console clean, particle burst confirmed visually on hover, `prefers-reduced-motion` respected by both CSS (`.shimmer-text` neutralized, existing universal animation-kill rule covers `.ambient-glow`) and JS (`Reveal` via `useReducedMotion()`, `ParticleButton` via a `matchMedia` early-return). See MEMORY.md for a significant false-alarm investigation: the Reveal component initially appeared completely broken (every scroll-reveal stuck at `opacity:0` forever) — traced through several hypotheses (HMR staleness, mobile-touch-emulation, viewport-margin math) before confirming the real cause was this session's browser-automation tool only processing `IntersectionObserver` callbacks on an explicitly forced paint/screenshot, not continuously like a real browser — the actual implementation was correct all along, confirmed by forcing a paint at the right scroll position and watching `opacity:1` land.
